...
def myART(ordered_projections, lr, num_iterations=100):
    ...
    for iteration in range(num_iterations):
        ...
        for i in range(num_projections):
            ...
            reconstructed_image[coords[0], coords[1]] += correction
    ...
    plt.axis('off')
    plt.savefig(f'reconstructed_image_lambda_{lr}.png')
    plt.close()
    return reconstructed_image


t = np.arange(-250, 250, 1)
...
exit()
myART(coords, lr=lr, num_iterations=10)
plt.plot(rrmses)
plt.xlabel('Iteration')
plt.ylabel('RRMSE')
plt.title('RRMSE vs Iteration')
plt.grid()
plt.savefig(f'rrmse_plot_lambda_{lr}.png')
